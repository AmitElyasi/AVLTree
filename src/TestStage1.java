...
public class TestStage1 {
    ...
    public boolean insert() {
        ...
        for (int i = 0; i < keys.length; i++) {
            if (avlTree.insert(items[i].getKey(), items[i].getInfo()) != rot[i]) {
                addError(String.format("problem at inserting [%d,\"%s\"], wrong rotation number", items[i].getKey(),
                        items[i].getInfo()), 3, 1);
                flag = false;
            }
        }
        ...
        if (avlTree.getRoot().getKey() != 2) {
            addError(String.format("root is supposed to be [%d,\"%s\"] and is [%d,\"%s\"] instead", 2, "2 # 2",
                    avlTree.getRoot().getKey(), avlTree.getRoot().getValue()), 3, 2);
            flag = false;
        }
        return flag;
    }
    ...
    public boolean delete() {
        ...
        // TEST 4.8
        avlTree = TestUtils.treefromKeys(new int[] { 1, 2, 3, 4 });
        avlTree.delete(2);
        if (avlTree.getRoot().getKey() == 2) {
            addError("does not maintain root", 4, 8);
            flag = false;
        }
    ...
    public boolean size() {
        ...
        if (avlTree.size() != 98) {
            addError(
                    "added 0-99, deleted 99 then 0 then 99 again(shouldn't affect). size expected is 98, got something else",
                    5, 3);
            flag = false;
        }
        // TEST 5.4
        ...
        if (avlTree.size() != 120) {
            addError(
                    "added 0-99, delted 0 and 99, added 200-251, deleted 220-249. expected size was 120, got something else",
                    5, 5);
            flag = false;
        }
        return flag;
    ...
    public boolean keysToArray() {
        ...
        if (!Arrays.equals(avlTree.keysToArray(), keys)) {
            addError(
                    "inserted 400-499, then 0-99, then 200-299. keys array wasn't [0,1,..,99,200,201,..,299,400,401,..,499]",
                    7, 4);
            flag = false;
        }
        return flag;
    ...
    public boolean infoToArray() {
        ...
        if (!Arrays.equals(avlTree.infoToArray(), infos)) {
            addError(
                    "inserted 400-499 then 0-99. info array wasn't [\"0 # 0\",\"1 # 1\",..,\"99 # 99\",\"401 # 401\",..,\"499 # 499\"]",
                    8, 3);
            flag = false;
        }
        ...
        if (!Arrays.equals(avlTree.infoToArray(), infos)) {
            addError(
                    "inserted 400-499, then 0-99, then 200-299. keys array wasn't [\"0 # 0\",\"1 # 1\",..,\"99 # 99\",\"200 # 200\",\n\t\t\"201 # 201\",..,\"299 # 299\",\"400 # 400\",\n\t\t\"401 # 401\",..,\"499 # 499\"]",
                    8, 4);
            flag = false;
        }
        return flag;
    ...
    public boolean treeListInsert() {
        TreeList treeList = new TreeList();
        treeList.insert(0, 5, "5");
        treeList.insert(1, 3, "3");
        treeList.insert(2, 4, "4");
        treeList.insert(3, 17, "17");
        treeList.delete(1);
        return true;
    }
